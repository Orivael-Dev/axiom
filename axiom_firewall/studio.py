...
class StudioConfig:
    ...
    quant_scheme: str  = "srd_alpha0"
    alpha:        float = 0.0    # SRD α: 0.0 → pure 4-bit, 0.5 → balanced, 1.0 → full residue
    group_size:   int   = 64     # block group size (32 | 64 | 128)
    top_k_pct:    float = 0.25   # fraction of layers to pack (0.10 | 0.25 | 0.50 | 1.0)
    gguf_quant:   str   = "Q4_K_M"  # GGUF output quant: Q4_K_M | Q5_K_M | Q6_K | Q8_0 | none
...
def config_from_dict(d: dict) -> StudioConfig:
    ...
    return StudioConfig(
        ...
        top_k_pct=float(d.get("top_k_pct", 0.25)),
        gguf_quant=d.get("gguf_quant", "Q4_K_M"),
    )
...
def _gguf_suffix(gguf_quant: str) -> str:
    return {"Q4_K_M": "q4km", "Q5_K_M": "q5km",
            "Q6_K": "q6k", "Q8_0": "q8_0"}.get(gguf_quant, "q4km")
...
def _pack_cell_code(cfg: StudioConfig, colab: bool) -> str:
    ...
    hw_comment = {
        "gpu":             "# Hardware: GPU (CUDA) — model loaded to GPU automatically",
        "cpu":             "# Hardware: CPU only — pack will be slower; set CUDA_VISIBLE_DEVICES='' to force",
        "compile_on_load": "# Hardware: compile_on_load — torch.compile() applied after load",
    }.get(cfg.hardware_map, "# Hardware: GPU (CUDA)")
    return f'''\
...
if AXM_PATH.exists():
    ...
    print(f"Packing {{MODEL_ID}} | α={cfg.alpha}  group_size={cfg.group_size}  top_k_pct={cfg.top_k_pct} ...")
    t0 = time.time()
    r = subprocess.run([
...
def _extract_cell_code(cfg: StudioConfig, colab: bool) -> str:
    if cfg.gguf_quant == "none":
        return '''\
# GGUF extraction skipped (gguf_quant=none selected in Studio).
# Run:  python3 axm_cli.py verify <axm_path>  to confirm the container.
print("GGUF extraction skipped — container-only mode.")
GGUF_PATH = None
'''
    suffix = _gguf_suffix(cfg.gguf_quant)
    n_gpu = "99" if cfg.hardware_map == "gpu" else "0"
    return f'''\
import subprocess, sys, time

AXM_TO_GGUF  = AXIOM_DIR / "research" / "quant" / "axm_to_gguf.py"
LLAMACPP_DIR = OUTPUT_DIR / "llama.cpp"
GGUF_PATH    = OUTPUT_DIR / (AXM_PATH.stem + "_{suffix}.gguf")
...
if GGUF_PATH.exists():
    ...
    print("Extracting AXM → GGUF {cfg.gguf_quant} ...")
    t0 = time.time()
    r = subprocess.run([
...
**Pipeline:** SRD α={cfg.alpha}  group_size={cfg.group_size}  top_k_pct={cfg.top_k_pct} → signed `.axm` → GGUF {cfg.gguf_quant}

**Model:** `{cfg.model_id}`  |  **Hardware:** {cfg.hardware_map}
...
def generate_python_script(cfg: StudioConfig) -> bytes:
    ...
    gguf_suffix = _gguf_suffix(cfg.gguf_quant)
    n_gpu_layers = "99" if cfg.hardware_map == "gpu" else "0"
    skip_extract_default = "True" if cfg.gguf_quant == "none" else "False"
    hw_comment = {
        "gpu":             "# Hardware: GPU (CUDA) — transformers loads to GPU automatically",
        "cpu":             "# Hardware: CPU only — set CUDA_VISIBLE_DEVICES='' before running",
        "compile_on_load": "# Hardware: compile_on_load — torch.compile() applied after load",
    }.get(cfg.hardware_map, "# Hardware: GPU (CUDA)")
...
Model:      {cfg.model_id}
Slots:      {", ".join(s.slot_type for s in cfg.slots) or "text only"}
SRD flags:  α={cfg.alpha}  group_size={cfg.group_size}  top_k_pct={cfg.top_k_pct}
GGUF quant: {cfg.gguf_quant}
Hardware:   {cfg.hardware_map}
...
def pack(output_dir: Path) -> Path:
    ...
    pack_script = AXIOM_DIR / "research" / "quant" / "pack_to_axm.py"
    print(f"Packing {{MODEL_ID}} | α={{ALPHA}}  group_size={{GROUP_SIZE}}  top_k_pct={{TOP_K_PCT}} ...")
    t0 = time.time()
    r = subprocess.run([
        sys.executable, str(pack_script),
        "--model",      MODEL_ID,
...
def extract(axm_path: Path, output_dir: Path, llamacpp_dir: Path):
    if GGUF_QUANT == "none":
        print("GGUF extraction skipped (gguf_quant=none).")
        return None

    gguf_path = output_dir / (axm_path.stem + "_{gguf_suffix}.gguf")
    if gguf_path.exists():
...
def smoke_test(gguf_path: Path, llamacpp_dir: Path):
    if gguf_path is None:
        print("⚠ No GGUF — skipping smoke test")
        return
    ...
    r = subprocess.run([
        ...
        "--n-gpu-layers", "{n_gpu_layers}", "--ctx-size", "256",
...
def main():
    ...
    p.add_argument("--skip-extract", action="store_true", default={skip_extract_default})
    ...
    if not args.skip_extract:
        ...
        if args.smoke_test and gguf_path:
...
def generate_json_config(cfg: StudioConfig) -> bytes:
    ...
    config = {
        ...
        "pack_command": (
            ...
            f"--alpha {cfg.alpha} "
            f"--top-k-pct {cfg.top_k_pct} "
            f"--group-size {cfg.group_size}"
        ),
        "verify_command": f"python3 axm_cli.py verify {model_slug}_srd.axm",
        "extract_command": (
            f"python3 research/quant/axm_to_gguf.py "
            f"--container {model_slug}_srd.axm "
            f"--gguf-out {model_slug}_{_gguf_suffix(cfg.gguf_quant)}.gguf "
            f"--quant {cfg.gguf_quant}"
        ) if cfg.gguf_quant != "none" else None,
    }
