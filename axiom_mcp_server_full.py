...
# These four tools reference capability modules (axiom_audit_ledger,
# axiom_marketplace, axiom_workspace, axiom_fusion) that are not present in this
# repo or any shipped image, so their handlers cannot run. Drop them from the
# advertised tool list and the dispatch table so the server only exposes tools
# that actually work. Re-add a name here once its module is implemented.
_UNAVAILABLE_TOOLS = {
    "axiom_ledger", "axiom_marketplace", "axiom_workspace", "axiom_fusion",
}
TOOLS = [_t for _t in TOOLS if _t["name"] not in _UNAVAILABLE_TOOLS]
for _n in _UNAVAILABLE_TOOLS:
    _HANDLERS.pop(_n, None)
