...
| 10 | Data & data governance | **Supported** | `AXIOM_DATA_GOVERNANCE.md`; teacher–student fairness + demographic-variant testing (`axiom/integrity_check.py`); hash-only logging (no raw PII); **data-subject-rights adapters** (`axiom_data_subject.py`) — access/portability/erasure/rectification with signed receipts (GDPR Art. 15/16/17/20) | Register application data stores; set retention + cross-border policy (doc §§7–9) |
...
| ~~Data-subject-rights hooks~~ ✅ closed | 10 / GDPR | **Done** — `axiom_data_subject.py`: access/portability/erasure/rectification adapters over Axiom stores; erasure crypto-shreds mutable PII while retaining append-only ledgers hash-only (resolves the erasure-vs-immutability tension); signed receipts | Register the deployer's own data stores via the `SubjectStore` protocol |
