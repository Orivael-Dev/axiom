...
# Force UTF-8 stdout/stderr so the "→"/"…" status glyphs don't crash on
# Windows consoles (cp1252). No-op where reconfigure is unavailable.
...
def build_legal_index(db_path: Path, corpus, *, batch: int = 2000,
    ...
    Note: indexing the corpus `title` field here (a static metadata token)
    was tested and slightly *hurt* BM25 (MRR 0.275->0.268) — the section
    titles add low-IDF generic terms and their useful words already appear
    in the body. Kept text-only deliberately.
    """
...
class SPLADEReranker:
    ...
    def __init__(self, model_name: str = "naver/splade-v3",
        ...
        self._model      = None
        self._ready      = False
        # Cache passage encodings by id: the same passages recur across
        # queries' candidate lists (and across tracks), so encode once.
        self._pcache: Dict[str, Dict[int, float]] = {}
    ...
    def rerank(
        ...
        for pid, txt in candidates:
            vec = self._pcache.get(pid)
            if vec is None:
                vec = self._encode(txt)
                self._pcache[pid] = vec      # encode each passage once
            scored.append((self._dot(q_vec, vec), pid))
